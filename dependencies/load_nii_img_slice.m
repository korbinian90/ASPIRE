...
%  modified for single slice keckstein

%  - Jimmy Shen (jimmy@rotman-baycrest.on.ca)

function [img,hdr] = load_nii_img_slice(hdr,filetype,fileprefix,machine,slice_idx,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB)

if ~exist('hdr','var') | ~exist('filetype','var') | ~exist('fileprefix','var') | ~exist('machine','var')
    error('Usage: [img,hdr] = load_nii_img(hdr,filetype,fileprefix,machine,[slice_idx],[img_idx],[dim5_idx],[dim6_idx],[dim7_idx],[old_RGB]);');
end
...
if length(unique(slice_idx)) ~= length(slice_idx)
    error('Duplicate image index in "slice_idx"');
end
...
[img,hdr] = read_image(hdr,filetype,fileprefix,machine,slice_idx,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB);

return					% load_nii_img


%---------------------------------------------------------------------
function [img,hdr] = read_image(hdr,filetype,fileprefix,machine,slice_idx,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB)
...
%  otherwise, load images that are specified in slice_idx, img_idx, 
%  dim5_idx, dim6_idx, and dim7_idx
%
%  For binary image, we have to read all because pos can not be
%  seeked in bit and can not be calculated the way below.
%
if hdr.dime.datatype == 1 | isequal(hdr.dime.dim(4:8),ones(1,4)) | ...
        (isempty(slice_idx) & isempty(img_idx) & isempty(dim5_idx) & isempty(dim6_idx) & isempty(dim7_idx))
        ...
        for i6=1:length(dim6_idx)
            for i5=1:length(dim5_idx)
                for t=1:length(img_idx)
                    for iS=1:length(slice_idx)
                    
                        %  Position is seeked in bytes. To convert dimension size
                        %  to byte storage size, hdr.dime.bitpix/8 will be
                        %  applied.
                        %
                        pos = sub2ind([d1 d2 d3 d4 d5 d6 d7], 1, 1, ...
                            slice_idx(iS), img_idx(t), dim5_idx(i5),dim6_idx(i6),dim7_idx(i7)) -1;
...
if ~isempty(slice_idx)
    hdr.dime.dim(5) = length(slice_idx);
end

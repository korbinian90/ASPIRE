% Modification to load one slice
% keckstein
...
%
function nii = load_nii_slice(filename, slice_idx, img_idx, dim5_idx, dim6_idx, dim7_idx, ...
			old_RGB, tolerance, preferredForm)

   if ~exist('filename','var')
      error('Usage: nii = load_nii(filename,[slice_idx], [img_idx], [dim5_idx], [dim6_idx], [dim7_idx], [old_RGB], [tolerance], [preferredForm])');
   end
...
%   nii.ext = load_nii_ext(filename);
   ...
   %
   [nii.img,nii.hdr] = load_nii_img_slice(nii.hdr,nii.filetype,nii.fileprefix, ...
		nii.machine,slice_idx,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB);
